...
public class TweetControllerTest {
	...
	mockMvc.perform(newTweet("Schibsted Spain",
		"We are Schibsted Spain (look at our home page http://www.schibsted.es/), we own Vibbo, InfoJobs, fotocasa, coches.net and milanuncios. Welcome! Some more characters are required to invalid the tweet."))
	...
	List<Tweet> list = new ObjectMapper().readValue(content, new TypeReference<List<Tweet>>() {
	});

	assertThat(list.size()).isEqualTo(1);

	mockMvc.perform(discardTweet(list.get(0).getId())).andExpect(status().is(204));
    }

    @Test
    public void shouldReturn400WhenDiscardingANonExistingTweet() throws Exception {
	mockMvc.perform(discardTweet(1L)).andExpect(status().is(404));
    }
	...
	getResult = mockMvc.perform(get("/discarded")).andExpect(status().is(200)).andReturn();
    ...
    private RequestBuilder discardTweet(Long id) {
